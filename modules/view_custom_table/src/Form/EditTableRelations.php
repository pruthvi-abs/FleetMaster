...
class EditTableRelations extends FormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($result) {
      \Drupal::messenger()->addStatus($this->t('@table relations are updated.', [
        '@table' => $table_name,
      ]));
    }
    else {
      \Drupal::messenger()->addError($this->t('Could not update @table data, please check log messages for error.', [
        '@table' => $table_name,
      ]));
    }
