...
class AddViewsCustomTable extends FormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($opeation == $this->t('Save')) {
      ...
      if ($result) {
        \Drupal::messenger()->addStatus($this->t('@table is added to views. Please clear cache to see changes.', [
          '@table' => $table_name,
        ]));
      }
      else {
        \Drupal::messenger()->addError($this->t('Could not add table to views, please check log messages for error.'));
      }
    }
